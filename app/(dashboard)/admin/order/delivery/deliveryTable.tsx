...
import { getOrderDelivery, updateDeliveryOrder } from "@/lib/db";
...
export default function DeliveryOrderTable({
  ...
  const orderModalRef = useRef<OrderModel | null>(null);
  const actionUpdate = updateDeliveryOrder.bind(null);
  const { toast } = useToast();
  useEffect(() => {
    let ignore = false;
    setOrderList([]);
    setIsLoading(true);
    getOrderDelivery(activeStatus.id, Number(user.id))
      .then((result) => {
        if (!ignore) {
          console.log(result);
          setOrderList(result);
                ...
                {isLoading ? (
                  ...
                  orderList.map((item) => (
                    <tr key={item.id} className="border-b border-gray-300">
                      ...
                      <td className="category_name px-6 py-3 line-clamp-2">
                        {item.diachinhanhang}
                      </td>
                      <td className="category_name px-6 py-3">
                        {item.chiTietHoaDons &&
                          ...
                          "Đang giao hàng" ? (
                            <>
                              <button
                                ...
                                onClick={() => {
                                  if (item.id && item.nhatKyDonHangs) {
      ...
      <Dialog open={openModal} onOpenChange={setOpenModal}>
        ...
        <DialogContent className="sm:max-w-[525px] bg-white">
          <DialogHeader>
            ...
            <DialogDescription>
              {`Bạn có chắc chắn muốn thay đổi đơn hàng này sang trạng thái ${orderModalRef.current?.title} không`}
            </DialogDescription>
          </DialogHeader>
          <form
            action={async () => {
              if (orderModalRef.current) {
                const res = await actionUpdate(
                  orderModalRef.current.orderId,
                  Number(user.id),
                  orderModalRef.current.statusId
                );
                if (res.status === 200) {
                  ...
                  toast({
                    title: `Thông báo`,
                    description: `Cập nhật đơn hàng không thành công. Vui lòng thực hiện lại!`,
                    action: (
                      <ToastAction className="border-none" altText="Try again">
                        <button className="inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-background transition-colors  focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 group-[.destructive]:border-muted/40 group-[.destructive]:hover:border-destructive/30 group-[.destructive]:hover:bg-destructive group-[.destructive]:hover:text-destructive-foreground group-[.destructive]:focus:ring-destructive">
                          Thử lại
